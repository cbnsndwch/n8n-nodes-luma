...
describe('Security Scanning Integration Tests', () => {
  ...
  describe('Security Configuration Files', () => {
    it('should have security policy configuration file', () => {
      ...
      const securityPolicy = JSON.parse(readFileSync(securityPolicyPath, 'utf-8'));
      ...
      expect(['low', 'moderate', 'high', 'critical']).toContain(securityPolicy.auditLevel);
      
      // Validate boolean flags
      expect(typeof securityPolicy.failOnVulnerabilities).toBe('boolean');
      expect(typeof securityPolicy.failOnLicenseViolations).toBe('boolean');
  ...
  describe('Package.json Security Scripts', () => {
    ...
    it('should include n8n license in allowed licenses', () => {
      ...
      expect(licenseScript).toContain('Custom: https://user-images.githubusercontent.com');
    });
  });
  ...
  describe('Workflow Integration', () => {
    it('should have security scanning in release workflow', () => {
      ...
      expect(releaseWorkflow).toContain('Release blocked due to security');
