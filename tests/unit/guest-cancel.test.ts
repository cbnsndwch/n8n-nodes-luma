...
describe('Guest Cancel Operation Unit Tests', () => {
  describe('Constants and Interfaces', () => {
    it('should have GUEST_CANCEL endpoint defined', async () => {
      const { LUMA_ENDPOINTS } = await import('../../dist/nodes/Luma/shared/constants.js');
    ...
    it('should have GuestCancellationData interface exported', async () => {
      ...
      try {
        ...
        throw new Error('GuestCancellationData interface compilation failed');
      }
    });
  });
  ...
  describe('Node Properties Structure', () => {
    it('should include cancel operation in guest operations', async () => {
      const { guestProps } = await import('../../dist/nodes/Luma/guest/props.js');
      ...
      const cancelOption = operationField?.options?.find(opt => opt.value === 'cancel');
    ...
    it('should have guestId field configured for cancel operation', async () => {
      ...
      const guestIdField = guestProps.find(prop => prop.name === 'guestId');
      expect(guestIdField).toBeDefined();
      expect(guestIdField?.displayOptions?.show?.operation).toContain('cancel');
    ...
    it('should have cancelledBy field for cancel operation', async () => {
      ...
      expect(cancelledByField?.displayOptions?.show?.operation).toContain('cancel');
      expect(cancelledByField?.options).toHaveLength(2);
      expect(cancelledByField?.options?.some(opt => opt.value === 'guest')).toBe(true);
      expect(cancelledByField?.options?.some(opt => opt.value === 'organizer')).toBe(true);
    });
  ...
  describe('Property Configuration Validation', () => {
    it('should have consistent field ordering', async () => {
      ...
      const operationIndex = guestProps.findIndex(prop => prop.name === 'operation');
      const guestIdIndex = guestProps.findIndex(prop => prop.name === 'guestId');
      const cancelledByIndex = guestProps.findIndex(prop => prop.name === 'cancelledBy');
